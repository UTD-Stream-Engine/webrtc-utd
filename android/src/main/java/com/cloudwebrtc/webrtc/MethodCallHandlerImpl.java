...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
  ...
  private void initialize(boolean bypassVoiceProcessing, boolean softwareVoiceProcessing, int networkIgnoreMask, boolean forceSWCodec, List<String> forceSWCodecList,
  @Nullable ConstraintsMap androidAudioConfiguration, Severity logSeverity, @Nullable Integer audioSampleRate, @Nullable Integer audioOutputSampleRate) {
    ...
    if(bypassVoiceProcessing) {
      ...
      // UTD fork. The hardware canceller off, and NOTHING ELSE changed.
      //
      // `bypassVoiceProcessing` above is the only switch upstream exposes that
      // reaches this builder, and it bundles five decisions: hardware AEC off,
      // hardware NS off, stereo in, stereo out, and a RAW MIC capture source
      // instead of VOICE_COMMUNICATION. The capture source is the part that
      // hurts: WebRTC's software canceller aligns the microphone against what
      // was played inside a bounded delay window, and the voice-communication
      // path is the low-latency one Android tunes for exactly that. Taking the
      // raw mic hands the software canceller a delay it cannot track — measured
      // on 2026-09-21 across one live room, three handsets on one build: 8-12 dB
      // cancelled on a Samsung, 0.77 dB frozen on a Redmi, nothing reported on a
      // third.
      //
      // Hardware cancellation is not the safe answer either. It is broken on a
      // long tail of devices that advertise it (Signal ships a blacklist for
      // exactly this), and on the media audio profile it does not engage at all.
      //
      // So: turn the hardware pair off so libwebrtc's APM is allowed to run, and
      // keep the voice-communication capture source, the low-latency path and
      // mono input that the APM needs to converge. This combination is what the
      // commercial SDKs ship as their own 3A, and there is no way to express it
      // through the upstream option set.
  ...
  public void onMethodCall(MethodCall call, @NonNull Result notSafeResult) {
    ...
    switch (call.method) {
      case "initialize": {
        ...
        // UTD fork: hardware AEC/NS off, voice-communication capture kept.
        boolean enableSoftwareVoiceProcessing = false;
        if(options.get("softwareVoiceProcessing") != null) {
          enableSoftwareVoiceProcessing = (boolean)options.get("softwareVoiceProcessing");
        }
        ...
        initialize(enableBypassVoiceProcessing, enableSoftwareVoiceProcessing, networkIgnoreMask, forceSWCodec, forceSWCodecList, androidAudioConfiguration, logSeverity, audioSampleRate, audioOutputSampleRate);
        result.success(null);
        break;
      }
